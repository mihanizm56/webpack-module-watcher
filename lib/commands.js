...
const getDockerRunCommand = () =>
  `stop ${dockerName} || docker run --name ${dockerName} -it --rm -d -v $(pwd)/nginx.conf:/etc/nginx/nginx.conf -v $(pwd)/build:/usr/share/metadata/core/build -p ${dockerPort}:80 fholzer/nginx-brotli nginx -g 'daemon off;'`;
